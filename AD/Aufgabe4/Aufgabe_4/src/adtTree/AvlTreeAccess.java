...
//			//System.out.println("Rechter Sohn: "+wurzel.rSohn.getKey());
	...
	public static void avltreeAccessKlauck() {
		AvlTreeAccess tree = AvlTreeAccess.create();
		AdtArray arr = adtTree.generate.Generator.importNums("zahlen_klauck_avl.dat");
		System.out.println(arr.length());
		long[] testErgArr = new long[2];
		
		for(int i = 0; i<=arr.length(); i++){
			testErgArr = tree.insert(arr.get(i));
		}
		exportArray("AvlTree", "Accessor", arr.length(), testErgArr);
		System.out.println("Gelesen: "+testErgArr[0] + " " + "Geschrieben: "+testErgArr[1]);
	}
