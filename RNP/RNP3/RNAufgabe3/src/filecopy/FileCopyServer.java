...
public class FileCopyServer {
    ...
    while (true) {
      try {
        ...
        if ((clientAdress.equals(receivedIPAddress)) &&
          ...
          System.out.println("Server: Packet has Length: "+ fcReceivePacket.getLen());
  ...
  private boolean setParameters(FCpacket controlPacket) {
    ...
    if (parameterArray.length == 3) {
      ...
      System.err.println("Control Packet (seqNum 0) has wrong number of parameters("+parameterArray.length+"): " +
                         parameters);
